...
export default function Login() {
  ...
  useEffect(() => {
    ...
    // Only redirect if user state is not set
    if (storedToken && storedUser && !user) {
      const parsedUser = JSON.parse(storedUser);
      setUser(parsedUser);

      // Check first login here as well
      if (parsedUser.is_first_logged_in) {
        navigate("/change_password", { state: { email: parsedUser.email } });
      } else {
        navigate("/dashboard");
      }
    }
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (response.token && response.user) {
        ...
        console.log(
          "first time login checked here",
          response.user.is_first_logged_in
        );
