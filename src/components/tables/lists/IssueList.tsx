...
const IssueTableColumns = [
  ...
  },
  {
    ...
    header: "Priority",
    cell: ({ row }: any) => <div>{row.getValue("priority.name") || "N/A"}</div>,
  },
  {
    accessorKey: "reporter.name",
    header: "Reporter",
    cell: ({ row }: any) => <div>{row.getValue("reporter.name") || "N/A"}</div>,
  },
  {
    accessorKey: "assignee.name",
    header: "Assignee",
    cell: ({ row }: any) => (
      <div>{row.getValue("assignee.name") || "Unassigned"}</div>
    ),
  },
  {
    id: "actions",
      ...
      return (
        <div className="flex items-center space-x-2">
          ...
          <Button
            ...
            onClick={() => {
              console.log("Delete issue:", issue.issue_id);
            }}
          >
