...
import AddWoreda from "../../../pages/woreda/woredaModal";
...
// Mock data
const mockCities: woreda[] = [
  {
    id: "org001",
    woreda_name:"Kirkos",
    subcity_name: "Addis Ababa",
  },
];
...
export default function CityTable() {
  ...
  const [editData, setEditData] = useState<woreda | null>(null);
  ...
  return mockCities.filter((subcity) =>
    subcity.woreda_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const totalPages = Math.ceil(filteredWoredas.length / entriesPerPage);
...
const handleFormSubmit = (values: Record<string, any>) => {
  if (editData) {
    ...
    setAlert({ type: "success", message: "Woreda updated successfully!" });
  } else {
    console.log("Adding:", values);
    setAlert({ type: "success", message: "Woreda added successfully!" });
  }
  closeModal();
    ...
    <>
      ...
      <div className="max-w-full overflow-x-auto">
        <div className="overflow-hidden rounded-xl border border-gray-200 shadow-sm">
          <Table>
            ...
            <TableBody>
              {paginatedWoredas.map((woreda, index) => (
                <TableRow key={woreda.id}>
                  ...
                  <TableCell className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => handleView(woreda.id)}
                        className="text-blue-500 hover:text-blue-600"
                      >
                        <EyeIcon className="w-5 h-5" />
                      </button>
                      <button
                        onClick={() => handleEdit(woreda.id)}
                        className="text-gray-600 hover:text-blue-700"
                      >
                        <PencilIcon className="w-5 h-5" />
                      </button>
                      <button
                        onClick={() => handleDelete(woreda.id)}
      ...
      <div className="flex items-center justify-between p-4 border-t border-gray-100 dark:border-gray-800">
        <div className="text-[#1E516A] text-sm">
          ...
          {Math.min(startIndex + entriesPerPage, filteredWoredas.length)}{" "}
          of {filteredWoredas.length} entries
        </div>
        <div className="flex items-center gap-2">
          <button
  ...
  <AddWoreda
    ...
    fields={[
      ...
      {
        id: "woreda_name",
        label: t("woreda.woreda_name"),
        type: "text",
        placeholder: "Enter Woreda name",
        value: editData?.woreda_name || "",
      },
