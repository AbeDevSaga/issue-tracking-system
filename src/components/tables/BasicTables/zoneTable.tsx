...
import AddZone from "../../../pages/city/cityModal";
...
export default function ZoneTable() {
  ...
  return mockZones.filter((zone) =>
    ...
    zone.region_name.toLowerCase().includes(searchTerm.toLowerCase())
  );
}, [searchTerm]);


  const startIndex = (currentPage - 1) * entriesPerPage;
  const paginatedCities = filteredZones.slice(
    startIndex,
    startIndex + entriesPerPage
  );
  const totalPages = Math.ceil(filteredZones.length / entriesPerPage);
...
const closeModal = () => {
  ...
  const handleView = (id: string) => {
    console.log("Viewing zone:", id);
  };

const handleEdit = (id: string) => {
  const org = mockZones.find((o) => o.id === id);
  console.log("org",org)
...
const handleFormSubmit = (values: Record<string, any>) => {
  if (editData) {
    ...
    setAlert({ type: "success", message: "Zone updated successfully!" });
  } else {
    console.log("Adding:", values);
    setAlert({ type: "success", message: "Zone added successfully!" });
  }
  closeModal();
    ...
    <>
      ...
      <div className="max-w-full overflow-x-auto">
        <div className="overflow-hidden rounded-xl border border-gray-200 shadow-sm">
          <Table>
            ...
            <TableBody>
              {paginatedCities.map((zone, index) => (
      ...
      <div className="flex items-center justify-between p-4 border-t border-gray-100 dark:border-gray-800">
        <div className="text-[#1E516A] text-sm">
          ...
          of {filteredZones.length} entries
        </div>
        <div className="flex items-center gap-2">
          <button
